...
2. 修改软件包关系
   ...
   - 包版本限定：
      ...
      - <<、<=、=、>= 和 >>，分别代表严格小于、小于或等于、严格等于、大于或等于以及严格大于,eg如下
...
1. 进入 [4.6] 中`uupdate`生成的目录"../\<spkg\>-\<oversion\>/",在此目录下打包
...
1. gpg: 找不到有效的 OpenPGP 数据

1. uscan: 错误: uupdate -f --upstream-version 1.0.6 subprocess returned exit status 1
